...
function M.setup()
  ...
  for extra, info in pairs(M.extras) do
    package.loaded["snappy.extra." .. extra] = nil
    local plugin = require("snappy.extra." .. extra)
    for style, style_name in pairs(styles) do
      snappy.setup({ style = style })
      snappy.load({ style = style })
      vim.cmd.colorscheme("snappy-" .. style)
      local colors = require("snappy.colors").setup({ transform = true })
      local fname = extra .. "/snappy_" .. style .. "." .. info.ext
      colors["_upstream_url"] = "https://github.com/folke/snappy.nvim/raw/main/extras/" .. fname
      colors["_style_name"] = "Tokyo Night" .. style_name
      colors["_name"] = "snappy_" .. style
